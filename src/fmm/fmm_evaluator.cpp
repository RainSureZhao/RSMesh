...
namespace rsmesh::fmm {
    ...
    public:
        ...
        void set_source_points_and_weights(const geometry::points3d& points,
            ...
            RSMESH_ASSERT(weights.rows() == points.rows());
        ...
        void set_weights(const Eigen::Ref<const valuesd>& weights) {
            RSMESH_ASSERT(weights.rows() == n_src_points_);
    ...
    private:
        [[nodiscard]] valuesd potentials() const {
            valuesd phi = valuesd::Zero(n_fld_points_);
    ...
    template<int Order>
    fmm_evaluator<Order>::fmm_evaluator(const rsmesh::model &model, int tree_height, const geometry::bbox3d &bbox) :pimpl_(std::make_unique<impl>(model, tree_height, bbox)) {
    ...
    template<int Order>
    valuesd fmm_evaluator<Order>::evaluate() const {
    ...
    template<int Order>
    void fmm_evaluator<Order>::set_source_points_and_weights(const geometry::points3d &points,
                                                             const Eigen::Ref<const valuesd> &weights) {
        pimpl_->set_source_points_and_weights(points, weights);
    }

    template<int Order>
    void fmm_evaluator<Order>::set_weights(const Eigen::Ref<const valuesd> &weights) {
